...
public class MidiRealTime {
    ...
    public static void main(String[] args) {
        ...
        try {
            String path = "/Users/agonc/Desktop/lacrymosa.midi";
    ...
    public static void convertMidi2RealTime(Sequence seq) {
	    ...
	    if (noteIsOff(nextEvent)) {
                ...
                System.out.println("track="+currentTrack+" tick="+tick+" time="+(int)(time+0.5)+"ms "
                        +" note "+((int)nextEvent.getMessage().getMessage()[1] & 0xFF)
                        +" chan "+((int)nextEvent.getMessage().getMessage()[0] & 0x0F)+ " off");
            } else if (noteIsOn(nextEvent)) {
                double time = (msb4+(((currentTempo/seq.getResolution())/1000)*(tick-tickOfTempoChange)));
                System.out.println("track="+currentTrack+" tick="+tick+" time="+(int)(time+0.5)+"ms "
                        +" note "+((int)nextEvent.getMessage().getMessage()[1] & 0xFF)
                        +" chan "+((int)nextEvent.getMessage().getMessage()[0] & 0x0F)+" on");
