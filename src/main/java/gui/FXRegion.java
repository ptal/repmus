...
public class FXRegion implements I_SimpleView{
	...
	public FXRegion (Region theNode) {
	...
	public FXRegion (Region theNode, boolean h, boolean v) {
		this(theNode);
		if (h || v){
	...
	public Point2D omViewSize() {
		if (scroller != null)
			return new Point2D (scroller.getPrefWidth(), scroller.getPrefHeight());
		else
	...
	public void omSetViewSize(double w, double h) {
		if (scroller != null) {
			scroller.setPrefWidth(w);
			scroller.setPrefHeight(h);
			delegate.setPrefWidth(Math.max(w, delegate.getPrefWidth()));
			delegate.setPrefHeight(Math.max(h, delegate.getPrefHeight()));
			}
		else
		{
	...
	public void omSetViewSize(Point2D size) {
		if (scroller != null) {
			scroller.setPrefWidth(size.getX());
			scroller.setPrefHeight(size.getY());
			delegate.setPrefWidth(Math.max(size.getX(), delegate.getPrefWidth()));
			delegate.setPrefHeight(Math.max(size.getY(), delegate.getPrefHeight()));
		}
		else
	...
	public Point2D omViewPosition() {
		if (scroller != null) 
			return new Point2D (scroller.getLayoutX(), scroller.getLayoutY());
		else 
	...
	public void omSetViewPosition(Point2D pos) {
		if (scroller != null) 
			scroller.relocate (pos.getX(),pos.getY());
		else 
